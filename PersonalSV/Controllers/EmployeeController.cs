    ...
    public class EmployeeController
    {
        ...
        public static bool AddOrUpdate(EmployeeModel model, EnumEditMode editMode)
        {
            ...
            using (var db = new PersonalDataEntities())
            {
                ...
                {
                    if (db.ExecuteStoreCommand("EXEC spm_InsertPersonal_2 @EmployeeCode, @EmployeeID, @EmployeeName, @Gender, @DepartmentName, @PositionName, @JoinDate, @NationID, @Address, @PhoneNumber, @Remark, @DayOfBirth",
                                                                          @EmployeeCode, @EmployeeID, @EmployeeName, @Gender, @DepartmentName, @PositionName, @JoinDate, @NationID, @Address, @PhoneNumber, @Remark, @DayOfBirth) >= 1)
                        result = true;
                    else result = false;
                }
                if (editMode == EnumEditMode.Update)
                {
                    if (db.ExecuteStoreCommand("EXEC spm_UpdatePersonal_2 @EmployeeCode, @EmployeeID, @EmployeeName, @Gender, @DepartmentName, @PositionName, @JoinDate, @NationID, @Address, @PhoneNumber, @Remark, @DayOfBirth",
                                                                          @EmployeeCode, @EmployeeID, @EmployeeName, @Gender, @DepartmentName, @PositionName, @JoinDate, @NationID, @Address, @PhoneNumber, @Remark, @DayOfBirth) >= 1)
                        result = true;
                    else result = false;
                }
